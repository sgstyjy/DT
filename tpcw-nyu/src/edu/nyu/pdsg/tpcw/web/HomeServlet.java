...
public class HomeServlet extends TPCWServlet {
    ...
    protected void serveRequest(HttpServletRequest req, PrintWriter out)  throws ServletException {
        ...
        for (int i = 0; i < Constants.NUM_SUBJECTS/2; i++) {
			out.println("<TR><TD><P ALIGN=\"center\"><A HREF=\"new_products?subject=" + Constants.SUBJECTS[i] + "\">" + Constants.SUBJECTS[i] + "</A></P></TD>");
			out.println("<TD><P ALIGN=\"center\"><A HREF=\"new_products?subject=" + Constants.SUBJECTS[i + Constants.NUM_SUBJECTS/2] + "\">" + Constants.SUBJECTS[i + Constants.NUM_SUBJECTS/2] + "</A></P></TD>");
			out.println("<TD BGCOLOR=\"#ffffff\" WIDTH=\"50\"></TD>");
			out.println("<TD><P ALIGN=\"center\"><A HREF=\"best_sellers?subject=" + Constants.SUBJECTS[i] + "\">" + Constants.SUBJECTS[i] + "</A></P></TD>");
			out.println("<TD><P ALIGN=\"center\"><A HREF=\"best_sellers?subject=" + Constants.SUBJECTS[i + Constants.NUM_SUBJECTS/2] + "\">" + Constants.SUBJECTS[i + Constants.NUM_SUBJECTS/2] + "</A></P></TD></TR>");
        }
        out.println("</TABLE>");
		...
		out.println("<A HREF=\"order_status\"><IMG SRC=\"images/order_status_B.gif\" ALT=\"Order Status\"></A>");
    }
