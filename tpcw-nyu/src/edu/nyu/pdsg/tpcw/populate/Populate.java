...
public class Populate {
	...
	private static void createTables() throws Exception {
		...
		statement = con.prepareStatement(Constants.CREATE_IDS_TABLE);
		statement.executeUpdate();
		System.out.println("Created table IDS");
	...
	private static void populateIdsTable() throws Exception {
		PreparedStatement statement = con.prepareStatement(Constants.INSERT_IDS);
		statement.executeUpdate();
		System.out.println("IDS table: done");
	}
	...
	private static void populateCustomerTable() throws Exception {
		...
		PreparedStatement statement = con.prepareStatement
		("INSERT INTO customer (C_ID, C_UNAME, C_PASSWD, C_FNAME, C_LNAME, C_ADDR_ID, C_PHONE, C_EMAIL, C_SINCE, C_LAST_VISIT, C_LOGIN, C_EXPIRATION, C_DISCOUNT, C_BALANCE, C_YTD_PMT, C_BIRTHDATE, C_DATA) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?)");
		...
		for (int i = 1; i <= Constants.NUM_CUSTOMERS; i++) {
			...
			statement.setString(7, C_PHONE);
			statement.setString(8, C_EMAIL);
			statement.setDate(9, C_SINCE);
			statement.setDate(10, C_LAST_VISIT);
			statement.setDate(11, C_LOGIN);
			statement.setDate(12, C_EXPIRATION);
